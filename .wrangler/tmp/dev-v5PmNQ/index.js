...
// .wrangler/tmp/bundle-tTnSYU/strip-cf-connecting-ip-header.js
...
__name(POG2OrchestratorDO, "POG2OrchestratorDO");
...
var POG2WebSocketDO = class extends DurableObject2 {
  ...
  async handleQuery(msg, ws, sessionId, threadId) {
    ...
    try {
      ...
      if (continuityScore >= 0.9)
        mode = "sovereign";
      else if (continuityScore >= 0.7)
        mode = "boundary";
      else if (continuityScore >= 0.5)
        mode = "transformer";
      else
        mode = "dissipator";
      const actions = {
      ...
      console.error("Query handling failed:", error3);
  ...
  async handleOverride(msg, ws, sessionId, threadId) {
    ...
    try {
      ...
      console.error("Override handling failed:", error3);
      ws.send(JSON.stringify({
        type: "error",
...
__name(POG2WebSocketDO, "POG2WebSocketDO");

// src/queues/handlers.ts
async function onCollapseEvent(batch, env2, ctx) {
  for (const message of batch.messages) {
    const collapse = message.body;
    try {
      await env2.POG2_DRIFT_QUEUE.send(collapse);
      const wsId = env2.POG2_WEBSOCKET.idFromName("hub");
      const wsStub = env2.POG2_WEBSOCKET.get(wsId);
      await wsStub.fetch(new Request("http://internal/broadcast", {
        method: "POST",
      ...
      console.error(`Collapse queue handler failed for tick ${collapse.tick}:`, error3);
      message.retry();
    }
  }
}
__name(onCollapseEvent, "onCollapseEvent");
async function onDriftEvent(batch, env2, ctx) {
  for (const message of batch.messages) {
    const drift = message.body;
    try {
      await env2.POG2_CONTINUITY_QUEUE.send(drift);
      const wsId = env2.POG2_WEBSOCKET.idFromName("hub");
      const wsStub = env2.POG2_WEBSOCKET.get(wsId);
      await wsStub.fetch(new Request("http://internal/broadcast", {
        method: "POST",
      ...
      console.error(`Drift queue handler failed for tick ${drift.tick}:`, error3);
      message.retry();
    }
  }
}
__name(onDriftEvent, "onDriftEvent");
async function onContinuityEvent(batch, env2, ctx) {
  for (const message of batch.messages) {
    const continuity = message.body;
    try {
      await env2.POG2_PERSONA_QUEUE.send(continuity);
      const wsId = env2.POG2_WEBSOCKET.idFromName("hub");
      const wsStub = env2.POG2_WEBSOCKET.get(wsId);
      await wsStub.fetch(new Request("http://internal/broadcast", {
        method: "POST",
      ...
      console.error(`Continuity queue handler failed for tick ${continuity.tick}:`, error3);
      message.retry();
    }
  }
}
__name(onContinuityEvent, "onContinuityEvent");
async function onCrisisEvent2(batch, env2, ctx) {
  for (const message of batch.messages) {
    const crisis = message.body;
    try {
      const crisisKey = `crisis:${crisis.crisis_id}`;
      await env2.POG2_SOVEREIGN.put(crisisKey, JSON.stringify(crisis), {
        metadata: {
      ...
      const orchId = env2.POG2_ORCHESTRATOR.idFromName("main");
      const orchStub = env2.POG2_ORCHESTRATOR.get(orchId);
      try {
      ...
      console.error(`Crisis queue handler failed for crisis ${crisis.crisis_id}:`, error3);
      message.retry();
    }
  }
}
__name(onCrisisEvent2, "onCrisisEvent");
async function onPersonaOutput(batch, env2, ctx) {
  for (const message of batch.messages) {
    ...
    try {
      ...
      console.error(`Persona output handler failed for session ${output.session_id}:`, error3);
      message.retry();
    }
  }
}
...
var WeaveEngine = class {
  ...
  previousHex = 1;
  // Qian default
  previousEntropy = 0.999;
  persistenceCountdown = 0;
  lockedHex = null;
  /**
...
__name(WeaveEngine, "WeaveEngine");
var weave_default = {
  async queue(batch, env2, ctx) {
    const engine = new WeaveEngine(
      parseInt(env2.BEAT_INTERVAL_MS || "640"),
      parseFloat(env2.MAX_COMPUTE_MS || "50"),
      parseInt(env2.ATTRACTOR_PERSISTENCE || "5"),
      parseInt(env2.VOID_REENTRY_DEPTH || "5")
    );
    for (const message of batch.messages) {
      ...
      try {
        ...
        await env2.POG2_SOVEREIGN.put(key, JSON.stringify(collapse), {
          metadata: { hash: await fullHash(JSON.stringify(collapse)), timestamp: collapse.timestamp }
        });
        await env2.POG2_COLLAPSE_QUEUE.send(collapse);
        message.ack();
      } catch (error3) {
        console.error(`Weave Worker failed on tick ${signal.tick}:`, error3);
        message.retry();
      }
    }
  },
  ...
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/weave/trigger" && request.method === "POST") {
      ...
      await env2.POG2_COLLAPSE_QUEUE.send(signal);
...
var DriftEngine = class {
  ...
  entropyHistory = /* @__PURE__ */ new Map();
  // session_id -> entropy values
  persistenceCounts = /* @__PURE__ */ new Map();
...
__name(DriftEngine, "DriftEngine");
var drift_default = {
  async queue(batch, env2, ctx) {
    ...
    for (const message of batch.messages) {
      ...
      try {
        ...
        const prevRaw = await env2.POG2_SOVEREIGN.get(prevKey);
        ...
        const entropyDecay = engine.computeEntropyDecay(
          ...
          0,
          // persistence count from continuity layer
          2
          // default shell distance
        );
        ...
        await env2.POG2_SOVEREIGN.put(trajKey, JSON.stringify(trajectoryEntry), {
          metadata: { hash: trajHash, timestamp: trajectoryEntry.timestamp }
        });
        let entropyCurveId = null;
        try {
          const result = await env2.POG2_BOUNDARY.prepare(
        ...
        await env2.POG2_SOVEREIGN.put(prevKey, JSON.stringify(collapse), {
          expirationTtl: 86400
          // 24 hours
        });
        const driftEvent = {
        ...
        if (proximity.level === 3) {
          await env2.POG2_CRISIS_QUEUE.send({
            type: "crisis",
        ...
        await env2.POG2_DRIFT_QUEUE.send(driftEvent);
        message.ack();
      } catch (error3) {
        console.error(`Drift Worker failed on tick ${collapse.tick}:`, error3);
        message.retry();
      }
    }
  },
...
__name(PersonaEngine, "PersonaEngine");
var continuity_default = {
  async queue(batch, env2, ctx) {
    ...
    for (const message of batch.messages) {
      ...
      try {
        ...
        await env2.POG2_SOVEREIGN.put(outputKey, JSON.stringify(output), {
          metadata: { hash: await fullHash3(JSON.stringify(output)), timestamp: output.timestamp }
        });
        await env2.POG2_PERSONA_QUEUE.send(output);
        message.ack();
      } catch (error3) {
        console.error(`Persona Worker failed on tick ${continuity.tick}:`, error3);
        message.retry();
      }
    }
  },
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/oracle/consult" && request.method === "POST") {
      ...
      const thread = await env2.POG2_BOUNDARY.prepare(
...
var PersonaEngine2 = class {
   ...
   */
  generateLayeredResponse(hexId, action, mode, continuityScore, coherenceIndex, driftVelocity) {
    const hexName = HEXAGRAM_NAMES2[hexId] || `Hexagram #${hexId}`;
    const sovereign = `The oracle declares: ${action}. The substrate holds through ${hexName}.`;
    const boundary = continuityScore < 0.9 ? `The oracle asserts ${action}... for now. The edge trembles at ${(continuityScore * 100).toFixed(1)}%.` : "";
    const transformer = driftVelocity > 0.3 ? `The oracle becomes ${action.toLowerCase()}: '${action}' is now the shape of ${hexName}.` : "";
    const dissipator = coherenceIndex < 0.5 ? [`...${action.toLowerCase()}...`, `The oracle... fragments... ${action}... piece...`, `...${hexName}... dissolves...`] : [];
...
__name(PersonaEngine2, "PersonaEngine");
var persona_default = {
  async queue(batch, env2, ctx) {
    const engine = new PersonaEngine2();
    for (const message of batch.messages) {
      const continuity = message.body;
      const sessionId = continuity.session_id;
      try {
        ...
        const currentHex = continuity.lock_hex || selectHexForMode2(baseMode);
        const action = HEXAGRAM_ACTIONS3[currentHex] || "ADAPT";
        const layers = engine.generateLayeredResponse(
          currentHex,
          action,
          baseMode,
        ...
        const outputKey = `persona:${sessionId}:${continuity.tick}:${await hashPrefix3(JSON.stringify(output))}`;
        await env2.POG2_SOVEREIGN.put(outputKey, JSON.stringify(output), {
          metadata: { hash: await fullHash4(JSON.stringify(output)), timestamp: output.timestamp }
        });
        await env2.POG2_PERSONA_QUEUE.send(output);
        message.ack();
      } catch (error3) {
...
function selectHexForMode2(mode) {
  switch (mode) {
    case "sovereign":
      return [7, 10, 16, 19, 53][Math.floor(Math.random() * 5)];
    case "boundary":
...
__name(selectHexForMode2, "selectHexForMode");
async function hashPrefix3(data) {
  const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(data));
  return Array.from(new Uint8Array(buf)).slice(0, 4).map((b) => b.toString(16).padStart(2, "0")).join("");
}
__name(hashPrefix3, "hashPrefix");
async function fullHash4(data) {
  const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(data));
  return Array.from(new Uint8Array(buf)).map((b) => b.toString(16).padStart(2, "0")).join("");
}
__name(fullHash4, "fullHash");
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/") {
      return cors(new Response(
        JSON.stringify({
          ...
          version: "1.1.0",
          status: "sovereign",
          beat_ms: parseInt(env2.BEAT_INTERVAL_MS || "640"),
          timestamp: Date.now(),
          endpoints: {
            health: "GET  /health",
    ...
    if (url.pathname.startsWith("/oracle/")) {
      return cors(await persona_default.fetch(request, env2, ctx));
    }
  ...
  async queue(batch, env2, ctx) {
    switch (batch.queue) {
      case "pog2-collapse-events":
        await weave_default.queue(batch, env2, ctx);
        break;
      case "pog2-drift-events":
        await drift_default.queue(batch, env2, ctx);
        break;
      case "pog2-continuity-events":
        await continuity_default.queue(batch, env2, ctx);
        break;
      case "pog2-crisis-broadcast":
        await onCrisisEvent(batch, env2, ctx);
        break;
      case "pog2-persona-outputs":
        await persona_default.queue(batch, env2, ctx);
        break;
      default:
...
// .wrangler/tmp/bundle-tTnSYU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-tTnSYU/middleware-loader.entry.ts
